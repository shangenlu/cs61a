...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        if not (isinstance(insect, QueenAnt) and not insect.impostor):
            insect.place = None
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        # BEGIN Problem 7
        return not (self.place.ant is None or self.place.ant.blocks_path == False)
        # END Problem 7
    ...
    def action(self, colony):
        ...
        # BEGIN EC
        "*** YOUR CODE HERE ***"
        if self.direction < 1 and self.place.entrance:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while place is not hive:
            if x >= self.min_range and x <= self.max_range:
                near_bee = random_or_none(place.bees)
                if near_bee:
                    return near_bee
...
'''
    def nearest_bee(self, hive):
        ...
        place = self.place
        spots = 0
        while place is not hive and spots <= self.max_range:
            near_bee = random_or_none(place.bees)
            if near_bee:
                return near_bee
            place = place.entrance
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor <= 0:
            place = self.place
            orig_bees = place.bees
            copy = orig_bees[:]
            for bee in copy:
                bee.reduce_armor(self.damage)
            place.remove_insect(self)
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        ...
        if self.digesting > 0:
            ...
            bees_array = self.place.bees
            if(bees_array):
                rand_bee = bees_array[random.randint(0, len(bees_array)-1)]
...
class WallAnt(Ant):
    ...
    def __init__(self, armor=4):
        """Create a WallAnt with an ARMOR quantity."""
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        # BEGIN Problem 13
        "*** YOUR CODE HERE ***"
        self.impostor = QueenAnt.impostor
    ...
    def action(self, colony):
        ...
        if self.impostor:
            ...
            while place.exit:
                ...
                if ant:
                    if ant.is_container:
                        if ant.contained_ant:
                            if ant.contained_ant not in self.buffed_ants:
                                self.buffed_ants.append(place.ant.contained_ant)
...
def apply_effect(effect, bee, duration):
    ...
    def new_action(colony):
        ...
        if duration > 0:
            duration -= 1
            effect(old_action, bee)(colony)
            #print(bee.place.name, time, colony.time)
        else:
            old_action(colony)
...
scary.action(colony)
bee.action(colony)
print(bee.place.name) # ScaryThrower should scare for two turns
bee.action(colony)
print(bee.place.name) # ScaryThrower should scare for two turns
bee.action(colony)
