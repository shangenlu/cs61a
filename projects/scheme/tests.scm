...
(eval (define kinetic_energy (/ (* 5 5) 2)))
; expect 12.5
...
(define (cos x specificity) ;;You can choose how far into the tayl(o)r series you want to go
    (let ((negative_square (* x x -1))(double (* 2 specificity)))
      (define (helper i a b)
        (if (= i double) (+ a b)
          (helper (+ i 2)
            (/ (* a negative_square) (+ i 1) (+ i 2))
              (+ a b))))
    (helper 0 1 0)))
; expect cos

(cos 1 100)
; expect 0.5403023058681397
