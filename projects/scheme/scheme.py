...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        operands = rest.map(lambda function: scheme_eval(function, env))
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        "*** YOUR CODE HERE ***"
        child = Frame(self)
        if not formals.__len__() == vals.__len__():
            raise SchemeError('arguments do not match up')
        while not formals == nil:
...
def do_and_form(expressions, env):
    ...
    if expressions == nil:
        return True
    while not expressions.second == nil:
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 15
    "*** YOUR CODE HERE ***"
    #check_form(bindings.first, 2)
    symbols = nil
    args = nil
    for i in range(bindings.__len__()):
        check_form(bindings.first, 2, 2)
        symbol = bindings.first.first
        arg = scheme_eval(bindings.first.second.first, env)
        symbols = Pair(symbol, symbols)
        args = Pair(arg, args)
        check_formals(symbols)
        bindings= bindings.second
    #check_formals(formals)
    return env.make_child_frame(symbols, args)
