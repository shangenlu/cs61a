...
def replace_leaf(t, old, new):
    ...
    names = [replace_leaf(b, old, new) for b in branches(t)]
    return tree(label(t), names)
...
def balanced(m):
    ...
    if is_weight(end(left(m))) and is_weight(end(right(m))):
        ...
        lefts = [balanced(end(right(b))) for b in [m] if is_mobile(end(right(b)))]
        rights = [balanced(end(left(b))) for b in [m] if is_mobile(end(left(b)))]
        if False not in lefts + rights:
            if total_weight(end(left(m)))*length(left(m)) == total_weight(end(right(m)))*length(right(m)):
                return True
            else:
                return False
...
def totals_tree(m):
    ...
    if is_weight(m):
        ...
        return tree(total_weight(m), [totals_tree(end(left(b))) for b in [m]] + [totals_tree(end(right(b))) for b in [m]])
...
def make_withdraw(balance, password):
    ...
    def withdraw(amount, input_password):
        nonlocal balance
        nonlocal incorrect_tries
        if len(incorrect_tries) >= 3:
            return "Your account is locked. Attempts: " + str(incorrect_tries)
        if input_password != password:
            incorrect_tries += [input_password]
...
def make_joint(withdraw, old_password, new_password):
    ...
    if type(string) == str:
        return string
    def joint_account(withdrawal, input_password):
        if input_password == new_password or input_password == old_password:
            return withdraw(withdrawal, old_password)
        else:
            return withdraw(withdrawal, input_password)
